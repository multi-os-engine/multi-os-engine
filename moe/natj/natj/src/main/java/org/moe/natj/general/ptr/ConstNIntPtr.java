...
/**
 * Constant architecture dependent signed Long pointer interface.
 * - On a 32-bit platform the element size is 4 bytes
 * - On a 64-bit platform the element size is 8 bytes
 */
public interface ConstNIntPtr extends ConstPtr<Long> {
    ...
    @Override
    public ConstNIntPtr ofs(int elemOffset);

    @Override
    public ConstNIntPtr getGuarded(int length);

    @Override
    public ConstNIntPtr getGuarded(int fromIndex, int toIndex);
