...
class NUIntPtrImpl extends VoidPtrImpl implements NUIntPtr {

    protected static final int ELEM_SIZE = CRuntime.POINTER_SIZE;

    // Reserved for NatJ runtime
    protected NUIntPtrImpl(Pointer peer) {
        super(peer);
    }

    // For ofs and java.nio.Buffer creation
    protected NUIntPtrImpl(long peer, Object bufferOwner) {
        super(peer, false, bufferOwner);
    }

    // For public creation
    NUIntPtrImpl(int capacity, boolean owned) {
    ...
    @Override
    public NUIntPtr ofs(int elemOffset) {
        return new NUIntPtrImpl(getRoot() + elemOffset * ELEM_SIZE, this);
    }
    ...
    @Override
    public final NUIntPtr getGuarded(int length) {
        return getGuarded(0, length);
    }

    @Override
    public NUIntPtr getGuarded(int fromIndex, int toIndex) {
        if (toIndex < fromIndex) {
            throw new IllegalArgumentException();
        }
        return new GuardedNUIntPtrImpl(getRoot(), this, fromIndex, toIndex);
    }
    ...
    static class ConstNUIntPtrImpl extends NUIntPtrImpl {

        // Reserved for NatJ runtime
        protected ConstNUIntPtrImpl(Pointer peer) {
            super(peer);
        }

        // For ofs creation only
        protected ConstNUIntPtrImpl(long peer, Object bufferOwner) {
            super(peer, bufferOwner);
        }

        // For public creation
        ConstNUIntPtrImpl(int capacity, boolean owned) {
        ...
        @Override
        public final NUIntPtr ofs(int elemOffset) {
            return new ConstNUIntPtrImpl(getRoot() + elemOffset * ELEM_SIZE, this);
        }
        ...
        public NUIntPtr getGuarded(int fromIndex, int toIndex) {
            ...
            return new GuardedConstNUIntPtrImpl(getRoot(), this, fromIndex, toIndex);
        }

    }

    private static class GuardedNUIntPtrImpl extends NUIntPtrImpl implements IGuardedPtr {

        protected final int low;
        protected final int hi;

        // For ofs and guarded creation only
        public GuardedNUIntPtrImpl(long peer, Object bufferOwner, int low, int hi) {
            super(peer, bufferOwner);
        ...
        @Override
        public NUIntPtr ofs(int elemOffset) {
            return new GuardedNUIntPtrImpl(getRoot() + elemOffset * ELEM_SIZE, this, low
    ...
    private static class GuardedConstNUIntPtrImpl extends GuardedNUIntPtrImpl {

        // For ofs and guarded creation only
        public GuardedConstNUIntPtrImpl(long peer, Object bufferOwner, int low, int hi) {
        ...
        @Override
        public final NUIntPtr ofs(int elemOffset) {
            return new GuardedConstNUIntPtrImpl(getRoot() + elemOffset * ELEM_SIZE, this, low
                    + elemOffset, hi + elemOffset);
        }
