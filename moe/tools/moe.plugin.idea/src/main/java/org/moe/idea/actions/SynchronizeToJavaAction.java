    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        for (Project project : ProjectManager.getInstance().getOpenProjects()) {
            for (Module current : ModuleManager.getInstance(project).getModules()) {
                if (MOESdkPlugin.isValidMoeModule(current) && currentFile.startsWith(ModuleUtils.getModulePath(current))) {
                    moduleTmp = current;
                    break;
                }
            }
            ...
            for (Dictionary.Field<PBXObjectRef<? extends PBXObject>, PBXObject> field : fields) {
                if (field.value instanceof PBXFileReference){
                    ...
                    if (filePath.endsWith(".xib") || filePath.endsWith(".storyboard")) {
                        ...
                        File headerFile = new File(xcodeProjectFile.getParentFile(), groupPath + File.separator + filePath);
...
//              if(headerFile.getName().endsWith(".h")){
