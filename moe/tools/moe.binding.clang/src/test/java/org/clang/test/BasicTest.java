...
public class BasicTest {
    ...
    public static void runTest() {
        ...
        String cmdline[] = new String[] { "-x", "objective-c", "-arch", "armv7", "-fmessage-length=133", "-std=gnu99",
                "-fobjc-arc", "-fpascal-strings", "-DNS_BLOCK_ASSERTIONS=1", "-DOBJC_OLD_DISPATCH_PROTOTYPES=0",
                "-isysroot", SDK_PATH, "-miphoneos-version-min=8",
        };
    ...
    private static void indexDeclaration(VoidPtr client, CXIdxDeclInfo decl) {
        ...
        case CXIdxEntityKind.CXIdxEntity_ObjCClass: {
            if (clang_index_getObjCContainerDeclInfo(decl).kind()
                    != CXIdxObjCContainerKind.CXIdxObjCContainer_Interface) break;
            String name = clang_getCursorSpelling(
                    clang_index_getObjCInterfaceDeclInfo(decl).containerInfo().declInfo().cursor()).toString();
        ...
        case CXIdxEntityKind.CXIdxEntity_ObjCCategory: {
            String name = clang_getCursorSpelling(decl.cursor()).toString();
            String class_name = clang_getCursorSpelling(clang_index_getObjCCategoryDeclInfo(decl).classCursor())
                    .toString();
        ...
        case CXIdxEntityKind.CXIdxEntity_ObjCClassMethod: {
            ...
            if (!container.equals(element_class) && !container.equals(element_protocol) && !container
                    .equals(element_category)) break;
