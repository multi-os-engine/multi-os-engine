...
final class Communication {
	 ...
	 *
	 * @param packet
	 *            packet containing the two-digit hex encoded string
	 * @param start
	 *            index to decoded from (inclusive)
	 * @param end
	 *            index to decoded to (exclusive)
	...
	boolean getAck() throws IOException {
		...
		while (ackQueue.size() == 0) {
			if (inputProcessingThread == null
					|| !inputProcessingThread.isAlive()) {
				throw new IOException(
						"waiting for ack failed, connection closed");
			}
			try {
				Thread.sleep(100);
	 ...
	 *
	 * @param noAckMode
	 *            <code>true</code> will turn on noAckMode.
	 */
	...
	String nextResponse() throws IOException {
		while (responsePacketQueue.size() == 0) {
			if (inputProcessingThread == null
					|| !inputProcessingThread.isAlive()) {
				throw new IOException(
						"waiting for response failed, connection closed");
			}
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
