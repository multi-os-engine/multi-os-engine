...
public class ObjCClassManager extends AbstractUnitManager {
	...
	static {
		...
		methTResolver.setSupportedMappers(new String[] {
				Constants.CStringMapper, Constants.ObjCStringMapper, Constants.CStringArrayMapper
		});
	...
	public void preparationPhase_Initialize() {
		methPathMatch[0] = this.getClass() == ObjCProtocolManager.class ? TypeConfiguration.OBJC_PROTOCOL
	 ...
	 */
	protected final String methPathMatch[] = new String[] {
			null, null, null, null, TypeConfiguration.LOCATION, null
	};
		...
		if (!method.isSupported(getGenerator(), problems)) {
			...
			if (isCreatorMethod(method)) {
				if (!methCreatorRetTResolver.supports(new Type(getUnitName()), false)) {
					problems.add(methCreatorRetTResolver.getName() + " resolver doesn't handle type ["
							+ method.getType() + "]");
					isValid = false;
				}
			} else {
			...
			for (CalleeArgument arg : method.getArguments()) {
				if (arg.getType().isCallback()) {
					...
					if (!cbman.isSupported()) {
						problems.add("unhadled callback at arg(" + arg.getName() + "@" + idx + ") type ["
								+ arg.getType() + "]");
						isValid = false;
					}
				}
	 ...
	 * 
	 * @param container
	 *            container to add the init methods to
	 * @param isExternal
	 *            method is external
	 * @throws GeneratorException
	 */
	 ...
	 *
	 * @param container
	 *            container to add the static methods to
	 * @param isExternal
	 *            method is external
	 * @throws GeneratorException
	 */
	 ...
	 * 
	 * @param container
	 *            container to add the factory methods to
	 * @param isExternal
	 *            method is external
	 * @throws GeneratorException
	 */
	 ...
	 * 
	 * @param protocol
	 *            protocol to add all methods from
	 */
	 ...
	 * 
	 * @param protocol
	 *            manager of the implemented protocol
	 */
	 ...
	 *            method
	 * @param cme
	 *            class member editor of the method's owner
	 * @throws GeneratorException
	 */
	private void update(ObjCMethod method, ClassMemberEditor cme) {
		...
		try {
			...
			if (editor != null && editor.isEditable()) {
				...
				if (method.getPropertyName() == null) {
					...
					if (!method.isProtocolMethod()) {
						type = method.isStatic() ? XcodeDocumentation.DocClassMethodType
			...
			if (method.getOwner() == this) {
				...
				methPathMatch[2] = method.isStatic() ? TypeConfiguration.CLASS_METHOD
