...
import static org.clang.c.clang.clang_disposeTokens;
import static org.clang.c.clang.clang_getFileName;
import static org.clang.c.clang.clang_tokenize;
...
public final class ClangUtil {
	...
	public static int[] getLocation(CXCursor cursor) {
		...
		return new int[] {
				line.getValue(), column.getValue(), offset.getValue()
		};
	}
	 ...
	 * 
	 * @param begin
	 *            start location in which to search
	 * @param end
	 *            end location
	...
	public static boolean isOptional(CXTranslationUnit tu, CXSourceLocation begin, CXSourceLocation end) {
		...
		@SuppressWarnings("unchecked")
		Ptr<Ptr<CXToken>> tokens_ref = (Ptr<Ptr<CXToken>>)PtrFactory.newPointerPtr(CXToken.class, 2, 1, true, false);
	...
	public static boolean isAvailable(CXCursor c) {
		...
		return (cursorAvailability == CXAvailabilityKind.Available)
				|| (cursorAvailability == CXAvailabilityKind.Deprecated);
	}
