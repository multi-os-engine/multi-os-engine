...
public class ObjCMethod extends AbstractModelElement implements IParameterizedCallable {
	 ...
	 *             when trying to copy a non-init and non-factory method
	 */
		...
		if (family == ObjCMethodFamily.UNDEFINED) {
			if (isStatic) {
				...
				if (range != -1) {
					...
					if (remaining == ownerClassName.length()
							|| remaining_sel.startsWith(ownerClassName.substring(remaining))) {
	...
	public boolean isRetainedReturn() {
		if (isStatic) {
			...
			return firstWord.equals("copy")
					|| (getName().startsWith("mutableCopy") && clangFirstWord("mutableC".length()).equals("opy"));
		}
	}
