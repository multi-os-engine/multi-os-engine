...
public class ThirdPartyLibraryLinkExecutor extends AbstractLinkExecutor {

    private Map<String, String> flags;

    public ThirdPartyLibraryLinkExecutor(String packageName, String[] framework, String[] javaSource, String[] headers,
            String[] bundle, String outFile, String ldFlags) {
        super(packageName, framework, javaSource, headers, bundle, outFile, ldFlags);
    ...
    protected void copyLibrary(File destination, List<String> libList) throws CheckArchitectureException, IOException {
        for (String value : libList) {
            ...
            if (value != null && !value.isEmpty() && frameworkFile.exists()) {
                ...
                List<ArchitectureVerifier.CPU_TYPE> archs = ArchitectureVerifier.checkArchitectures(frameworkFile);
                ...
                if (archs.size() >= 4 || (arm && sim)) {
                    ...
                    FileUtils.copyFile(frameworkFile, destinationFr);
                    ...
                    File destinationLib = new File(destinationFr, frameworkFile.getName());
                    if (!destinationLib.exists()) {
                        destinationLib.createNewFile();
                    }
                    FileUtils.copyFile(frameworkFile, destinationLib);
                } else if (archs.size() == 1) {
                    ArchitectureVerifier.CPU_TYPE arch = archs.iterator().next();
                    File destinationFr = new File(destination, arch.getName());
                    if (!destinationFr.exists()) {
                        destinationFr.mkdirs();
                    }
                    ...
                    if (flags.containsKey(arch.getManifestConst())) {
                        ...
                        flags.put(arch.getManifestConst(), flagValue + ";" + String
                                .format("./lib/%s/%s", arch.getName(), frameworkFile.getName()));
                    } else {
                        flags.put(arch.getManifestConst(),
                                String.format("./lib/%s/%s", arch.getName(), frameworkFile.getName()));
                    }
                    ...
                    FileUtils.copyDirectory(frameworkFile, destinationLib);
                }
            }
        }
    }
