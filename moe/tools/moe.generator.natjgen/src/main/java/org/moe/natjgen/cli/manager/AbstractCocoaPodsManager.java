...
public abstract class AbstractCocoaPodsManager {
    ...
    protected List<File> getFolderList(String bundleWildcard, File unzipedFolder) {
        ...
        if (recursiveIdx >= 0) {
            ...
            pathWildcard =
                    unzipedFolder.getPath() + File.separator + convertGlobToRegEx(pathWildcard) + ".*" + otherDirPath;

            String rootFolder = unzipedFolder.getPath() + (recursiveIdx == 0 ?
                    "" :
                    File.separator + bundleWildcard.substring(0, recursiveIdx - 1)) + otherDirPath;
