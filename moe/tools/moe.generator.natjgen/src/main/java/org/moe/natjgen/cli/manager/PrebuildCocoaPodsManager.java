...
public class PrebuildCocoaPodsManager extends AbstractCocoaPodsManager {
    ...
    public IExecutor processCocoapods(String source, SpecObject spec, String packageName, String[] javaSource,
            String outputJar) throws IOException, CompressorException, ArchiveException, InvalidParameterSpecException,
            InterruptedException, URISyntaxException, UnsupportedTypeException {
        ...
        if (outFile.getName().endsWith(".zip") || (type != null && type.equals("zip"))) {
        ...
        } else if (outFile.getName().endsWith("tar.bzip2") || outFile.getName().endsWith("tar.gz") || outFile.getName()
                .endsWith("tar.bz2") || type.equals("tgz")) {
            ...
            ArchiveInputStream input = new ArchiveStreamFactory()
                    .createArchiveInputStream(ArchiveStreamFactory.TAR, inputC);
        ...
        if (spec.getVendoredFrameworks() != null && spec.getVendoredFrameworks().size() > 0) {
            ...
            for (String vFramework : spec.getVendoredFrameworks()) {
                ...
                if (frName.size() != 1) throw new RuntimeException(
                ...
                if (frameworkFile.exists()) {
                    ...
                    if (frameworkNameIdx >= 0) {
                        ldFlags =
                                ldFlags + "-framework " + frameworkFile.getName().substring(0, frameworkNameIdx) + ";";
                    }
                }
            }
            ...
            for (String lib : spec.getVendoredLibraries()) {
                ...
                if (libName.size() != 1) throw new RuntimeException(
                        "Something wrong with this code. Refactor it! And the same with libraries");
                ...
                if (library.exists()) {
                    ...
                    if (libNameIdx >= 0) {
                        ...
                        ldFlags = ldFlags + "-l" + (libShortName.startsWith("lib") ?
                                libShortName.substring(3) :
                                libShortName) + ";";
                    }
                }
            }
