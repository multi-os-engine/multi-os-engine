...
import org.moe.natjgen.cli.executable.Svn.ConnectionException;
import org.moe.natjgen.cli.executable.Svn.NotDirectoryException;
import org.moe.natjgen.cli.executable.Svn.RepositoryNotFoundException;
import org.moe.natjgen.cli.executable.Svn.SvnNotFoundException;
...
public class GrabUtils {
    ...
    public static void download(@NonNull URI link, @NonNull File dest)
            throws FileAlreadyExistsException, FileNotFoundException, UnsupportedTypeException, URISyntaxException,
            IOException {
     ...
     * @throws IOException                if operation couldn't be successfully completed because of other reasons
     */
    public static void downloadFileFromGit(@NonNull URI git, @Nullable String tag, @Nullable Path file,
            @NonNull File output)
        ...
        String svnCommandAddress =
                gitRepo.substring(0, gitRepo.length() - 4) + (tag == null ? "/trunk/" : "/tags/" + tag + "/") + (
