...
public class SourceCocoaPodsManager extends AbstractCocoaPodsManager {
    ...
    public IExecutor processCocoapods(String source, SpecObject spec, String packageName, String[] javaSource,
        ...
        if (dependenciesFramework.size() > 0) {
            ...
            executor = new ThirdPartyFrameworkLinkExecutor(packageName, frameworkParam.toArray(new String[0]),
                    javaSource, headerContent.toArray(new String[0]), bundleContent.toArray(new String[0]), outputJar,
                    ldFlags);
        }
    ...
    private String buildPods(SpecObject spec) throws WrapNatJGenException, IOException {
        ...
        Utils.execute(podsDir,
                "xcodebuild -target " + spec.getName() + " -sdk iphoneos -configuration Release -project "
                        + projectPath, null);
        String result = Utils.execute(new File(JPod.getPath() + File.separator + "Pods"),
                "xcodebuild -target " + spec.getName() + " -sdk iphonesimulator -configuration Release -project "
                        + projectPath, null)[1];

        return result;
    }

    private List<SpecObject> getAllDepSpecs(SpecObject spec, List<String> exceptList)
            throws IOException, InterruptedException, WrapNatJGenException {
