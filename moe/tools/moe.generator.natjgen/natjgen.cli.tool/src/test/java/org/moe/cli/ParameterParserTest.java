...
import org.moe.cli.executor.GenerateBindingExecutor;
import org.moe.cli.executor.IExecutor;
import org.moe.cli.executor.ThirdPartyFrameworkLinkExecutor;
import org.moe.cli.executor.ThirdPartyLibraryLinkExecutor;
...
public class ParameterParserTest extends WrapNatJTest {
	...
	public void generateBindings() throws Exception {
		...
		CommandLine argc = parseArgs(new String[] { "--path-to-project", project.getPath(), "--headers",
				header.getPath(), "--package-name", "org" });
		IExecutor executor = ExecutorManager.getExecutorByParams(argc);
		assertNotNull(executor);
		assertTrue(executor instanceof GenerateBindingExecutor);
	...
	public void linkUniversalLibrary() throws Exception {
		...
		CommandLine argc = parseArgs(new String[] { "--library", library.getPath(), "--headers",
				headers.getParentFile().getPath(), "--package-name", "org", "--output-file-path", outputJar.getPath(),
				"--ld-flags", ldFlags.getPath(), "--bundle", bundle.getPath() });
