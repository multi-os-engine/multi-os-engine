...
# MOE: Moved arch detection up, so in case of unsupported architecture we just
# forward everything to the compiler.
...
# MOE: Disabled arm support.
if ((not exists $canonical_arch{$arch}) or ($arch eq "arm")) {
    exec(@gcc_cmd);
} elsif (grep /\.c$/, @gcc_cmd) {
