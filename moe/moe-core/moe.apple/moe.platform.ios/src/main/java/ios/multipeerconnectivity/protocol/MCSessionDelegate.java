...
public interface MCSessionDelegate {
	...
	void sessionDidFinishReceivingResourceWithNameFromPeerAtURLWithError(
			MCSession session, String resourceName, MCPeerID peerID,
	...
	default void sessionDidReceiveCertificateFromPeerCertificateHandler(
			MCSession session,
			NSArray<?> certificate,
			MCPeerID peerID,
	...
	public interface Block_sessionDidReceiveCertificateFromPeerCertificateHandler {
		@Generated
		void call_sessionDidReceiveCertificateFromPeerCertificateHandler(
				boolean arg0);
	}
	...
	void sessionDidReceiveDataFromPeer(MCSession session, NSData data,
			MCPeerID peerID);
	...
	void sessionDidReceiveStreamWithNameFromPeer(MCSession session,
			NSInputStream stream, String streamName, MCPeerID peerID);
	...
	void sessionDidStartReceivingResourceWithNameFromPeerWithProgress(
			MCSession session, String resourceName, MCPeerID peerID,
			NSProgress progress);
	...
	void sessionPeerDidChangeState(MCSession session, MCPeerID peerID,
			@NInt long state);
}
