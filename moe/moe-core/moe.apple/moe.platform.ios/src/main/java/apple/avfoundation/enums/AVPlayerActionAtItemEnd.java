 ...
 * its queue.
 * This value is supported only for players of class AVQueuePlayer. An AVPlayer that's not an AVQueuePlayer will raise
 * an NSInvalidArgumentException if an attempt is made to set its actionAtItemEnd to AVPlayerActionAtItemEndAdvance.
 * [@constant] AVPlayerActionAtItemEndPause
 * Indicates that when an AVPlayerItem reaches its end time the player will automatically pause (which is to say, the
 * player's
 ...
 * player's rate
 * will not change, its currentItem will not change, and its currentTime will continue to be incremented or decremented
 * as time
 * elapses, according to its rate). After this, if the player's actionAtItemEnd is set to a value other than
