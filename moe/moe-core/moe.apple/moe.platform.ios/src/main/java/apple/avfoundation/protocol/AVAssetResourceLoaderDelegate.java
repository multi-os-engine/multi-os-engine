...
public interface AVAssetResourceLoaderDelegate {
     ...
     * -[AVAssetResourceLoadingRequest finishLoading] or -[AVAssetResourceLoadingRequest finishLoadingWithError:]. If
     * you intend to finish loading the resource after your handling of this message returns, you must retain the
     * instance of AVAssetResourceLoadingRequest until after loading is finished.
     * If the result is NO, the resource loader treats the loading of the resource as having failed.
     * Note that if the delegate's implementation of -resourceLoader:shouldWaitForLoadingOfRequestedResource: returns
     ...
     * resourceLoader:shouldWaitForLoadingOfRequestedResource:.
     * If the result is YES, the resource loader expects invocation, either subsequently or immediately, of either
     * -[AVAssetResourceRenewalRequest finishLoading] or -[AVAssetResourceRenewalRequest finishLoadingWithError:]. If
     * you intend to finish loading the resource after your handling of this message returns, you must retain the
     * instance of AVAssetResourceRenewalRequest until after loading is finished.
     * If the result is NO, the resource loader treats the loading of the resource as having failed.
     * Note that if the delegate's implementation of -resourceLoader:shouldWaitForRenewalOfRequestedResource: returns
     * YES without finishing the loading request immediately, it may be invoked again with another loading request
     * before the prior request is finished; therefore in such cases the delegate should be prepared to manage multiple
     * loading requests.
     * 
     ...
     * immediately, to the NSURLAuthenticationChallenge's sender, i.e. [authenticationChallenge sender], via use of one
     * of the messages defined in the NSURLAuthenticationChallengeSender protocol (see NSAuthenticationChallenge.h). If
