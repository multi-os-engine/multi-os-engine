...
public class NSFileWrapper extends NSObject implements NSSecureCoding {
    ...
    @Owned
    @Selector("new")
    public static native NSFileWrapper new_objc();
    ...
    /**
     * Add a file wrapper to the receiver's children and return the file name that uniquely identifies it among the
     * receiver's children. The unique file name will be the same as the passed-in file wrapper's preferred file name
    ...
    /**
     * A designated initializer for creating an instance for which -isDirectory returns YES. The passed-in dictionary
     ...
     * directory, children are compared against the files in the directory, recursively. The contents of files are not
     * compared; matching of regular files is based on file modification dates. Because children of directory file
     * wrappers are not read immediately by -initWithURL:options:error: or -readFromURL:options:error: unless
     ...
     * attempt to use this name as the key into its dictionary of children. Usually the preferred file name will
     * actually get used in this situation but it won't be if that key is already in use. The default implementation of
     * this method causes existing parents to remove and re-add the child to accommodate the change. Preferred file
    ...
    /**
     * Recursively write the entire contents of the receiver at the specified location, and return YES if successful. If
     * not successful, return NO after setting *outError to an NSError that encapsulates the reason why the file wrapper
     * could not be written. originalContentsURL may be nil or it may be the location of a previous revision of what is
     * being written. If it is not nil the default implementation of this method attempts to avoid unnecessary I/O by
