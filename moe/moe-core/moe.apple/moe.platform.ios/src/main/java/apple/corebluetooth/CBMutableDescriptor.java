 ...
 * Descriptor types are detailed in @link CBUUID.h @/link, but only the <code>Characteristic User Description</code> and
 * <code>Characteristic Presentation
 * Format</code> descriptors are currently supported. The <code>Characteristic Extended Properties</code> and
...
public class CBMutableDescriptor extends CBDescriptor {
    ...
    @Generated
    protected CBMutableDescriptor(Pointer peer) {
        super(peer);
    }
    ...
    @Owned
    @Selector("new")
    public static native CBMutableDescriptor new_objc();
     ...
     * initWithType:value:
     * <p>
     * Returns a decriptor, initialized with a service type and value. The <i>value</i> is required and cannot be
     * updated dynamically
