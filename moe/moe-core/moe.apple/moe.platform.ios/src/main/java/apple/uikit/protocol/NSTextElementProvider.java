...
public interface NSTextElementProvider {
    ...
    /**
     * Enumerates the text elements starting at textLocation. If textLocation=nil, it assumes
     * self.documentRange.location for forward enumeration; uses self.documentRange.endLocation for reverse enumeration.
     * When enumerating backward, it starts with the element preceding the one containing textLocation. If enumerated at
     * least one element, it returns the edge of the enumerated range. Note that the enumerated range might not match
