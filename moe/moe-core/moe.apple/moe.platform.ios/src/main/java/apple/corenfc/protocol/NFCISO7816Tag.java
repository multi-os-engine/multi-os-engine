 ...
 * invalidation
 * method if the required entitlement is missing or "com.apple.developer.nfc.readersession.iso7816.select-identifiers"
 * does not contain
 * at least one valid entry.
 * When the reader discovered a compatible ISO7816 tag it automatically performs a SELECT command (by DF name) using the
 * values provided in
...
public interface NFCISO7816Tag extends NFCTag, NFCNDEFTag {
    ...
    /**
     * [@property] initialSelectedAID The Hex string of the application identifier (DF name) selected by the reader when
