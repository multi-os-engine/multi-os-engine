 ...
 * <p>
 * Defines an interface for delegates of AVCaptureDepthDataOutput to receive captured depth data and be notified of late
 * depth data that were dropped.
 */
@Generated
...
public interface AVCaptureDepthDataOutputDelegate {
     ...
     * <p>
     * Delegates receive this message whenever a depth data object is dropped. This method is called once for each
     * dropped depth data. The object passed to this delegate method will contain a shell of an AVDepthData that
     * contains no actual depth data backing pixel buffer, as well as a presentation time stamp and a reason for the
     * drop. This method will be called on the dispatch queue specified by the output's delegateCallbackQueue property.
     ...
     * @param depthData  A depth data object containing information about the dropped depth, such as its native depth
     *                   type. This depth data object produces nil CVPixelBuffers for depth / disparity as it has no
     *                   backing depth map.
     ...
     * <p>
     * The delegate receives this message whenever the depth data output captures and outputs a new depth data object.
     * This method is called on the dispatch queue specified by the output's delegateCallbackQueue property. This method
     * is called frequently. Care must be taken to process the depth data quickly in order to prevent dropped depth
     * data.
     * <p>
     * Clients that need to reference the AVDepthData object outside of the scope of this method must retain it and then
     * release it when they are finished with it (in a MRR app).
     * <p>
     * Note that to maintain optimal performance, AVDepthData pixel buffer maps may be backed by a finite memory pool.
     * If AVDepthData objects are held onto for too long, capture inputs will no longer be able to copy new depth data
