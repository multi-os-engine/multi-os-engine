...
public class NSTimer extends NSObject {
     ...
     * - parameter: ti The number of seconds between firings of the timer. If seconds is less than or equal to 0.0, this
     * method chooses the nonnegative value of 0.1 milliseconds instead
     * - parameter: repeats If YES, the timer will repeatedly reschedule itself until invalidated. If NO, the timer will
    ...
    /**
     * Creates and returns a new NSTimer object initialized with the specified block object. This timer needs to be
     * scheduled on a run loop (via -[NSRunLoop addTimer:]) before it will fire.
     * - parameter: timeInterval The number of seconds between firings of the timer. If seconds is less than or equal to
    ...
    /**
     * Initializes a new NSTimer object using the block as the main body of execution for the timer. This timer needs to
     * be scheduled on a run loop (via -[NSRunLoop addTimer:]) before it will fire.
     * - parameter: fireDate The time at which the timer should first fire.
     * - parameter: interval The number of seconds between firings of the timer. If seconds is less than or equal to
     * 0.0, this method chooses the nonnegative value of 0.1 milliseconds instead
     * - parameter: repeats If YES, the timer will repeatedly reschedule itself until invalidated. If NO, the timer will
     * be invalidated after it fires.
     * - parameter: block The execution body of the timer; the timer itself is passed as the parameter to this block
