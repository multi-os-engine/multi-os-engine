...
public interface UIScribbleInteractionDelegate {
    /**
     * Allows the delegate to prevent Scribble from starting at a specific location in the view. If not implemented,
     * defaults to YES.
     * You can use this callback to temporarily suppress Scribble in text input views if your app supports drawing over
     * text or other special interaction when using Apple Pencil. In cases like this, it's recommended to provide a UI
    ...
    /**
     * Allow the delegate to delay focusing (making first responder) the text input view. Normally, Scribble will focus
     * the target input as soon as the user begins writing, but if you return YES from this callback, it will wait until
     * the user makes a brief pause. This is useful in cases where the text input view will shift or transform when
