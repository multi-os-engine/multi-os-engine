...
public interface UIIndirectScribbleInteractionDelegate {
    ...
    /**
     * Asks the delegate to focus an element to handle text edits. In response, it should make the element the currently
     * focused one, and make the corresponding UITextInput become first responder.
     * If the element was not focused already, text selection should be set to the character location closest to
    ...
    /**
     * Allow the delegate to delay focusing an element. Normally, Scribble will focus the element as soon as the user
     * begins writing, but if you return YES from this callback, it will wait until the user makes a brief pause. This
     * is useful in cases where the element's frame will shift or transform when becoming focused, which can be
