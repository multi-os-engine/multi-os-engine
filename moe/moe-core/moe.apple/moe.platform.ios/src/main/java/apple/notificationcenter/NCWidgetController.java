...
/**
 * 'NCWidgetController' provides an interface available to both the widget and the widget-providing app through which
 * clients can specify whether the widget has content to display.
 * If the widget determines it no longer has content to display, it can obtain a widget controller and update this
 * state.
...
@ObjCClassBinding
public class NCWidgetController extends NSObject {
    static {
        NatJ.register();
    }

    @Generated
    protected NCWidgetController(Pointer peer) {
        super(peer);
    }
    ...
    public static native NCWidgetController allocWithZone(VoidPtr zone);
    ...
    @Owned
    @Selector("new")
    public static native NCWidgetController new_objc();
