...
public class AVPersistableContentKeyRequest extends AVContentKeyRequest {
     ...
     * as any subsequent requests for the same key url using processContentKeyResponse: method. When you receive an
     * AVContentKeyRequest via -contentKeySession:didProvideContentKeyRequest: and you want to use existing persistent
     * content key from storage, you must invoke -respondByRequestingPersistableContentKeyRequest on that
     * AVContentKeyRequest in order to signal that you want to process an AVPersistableContentKeyRequest instead. If the
    ...
    public native NSData persistableContentKeyFromKeyVendorResponseOptionsError(@NotNull NSData keyVendorResponse,
