...
public class VNGeometryUtils extends NSObject {
    ...
    /**
     * Calculates a bounding circle that includes a collection of points or a VNContour object. Note that because this
     * is based on a geometric shape the aspect ratio is important when using normalized points.
     * This takes the aspect ratio of the contour into account when using a VNContour as an input.
     * boundingCircleForPoints and boundingCircleForSIMDPoints assume that the aspect ratio is correctly applied to the
     * points.
     * 
    ...
    public static native VNCircle boundingCircleForPointsError(@NotNull NSArray<? extends VNPoint> points,
            @Nullable @ReferenceInfo(type = NSError.class) Ptr<NSError> error);

    /**
     * Calculates a closed contour area using Green's theorem. The contour is represented by a set of points in
     * VNContour object,
     * It's important to note that a random set of points, or a contour with self-crossing edges will likely produce
     * undefined results
     ...
     * 
     * @param orientedArea If true, returns signed area - positive for CCW oriented contours and negative for CW
     *                     oriented contours.
    ...
    public static native boolean calculateAreaForContourOrientedAreaError(@NotNull DoublePtr area,
            @NotNull VNContour contour, boolean orientedArea,
            @Nullable @ReferenceInfo(type = NSError.class) Ptr<NSError> error);

    /**
     * Calculates perimeter, or a sum of all arc-lengths (edges), of a closed contour. The contour is represented by a
     * set of points in VNContour object.
     * Note that because this is based on a geometric shape the aspect ratio is important when using normalized points.
     * This takes the aspect ratio of the contour into account when using a VNContour as an input.
     * 
    ...
    public static native boolean calculatePerimeterForContourError(@NotNull DoublePtr perimeter,
            @NotNull VNContour contour, @Nullable @ReferenceInfo(type = NSError.class) Ptr<NSError> error);
    ...
    @Owned
    @Selector("new")
    public static native VNGeometryUtils new_objc();
