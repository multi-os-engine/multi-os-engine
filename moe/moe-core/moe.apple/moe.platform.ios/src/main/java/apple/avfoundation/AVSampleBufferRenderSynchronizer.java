...
public class AVSampleBufferRenderSynchronizer extends NSObject {
     ...
     * undefined behavior.
     *
     * @return An object conforming to the NSObject protocol. You must retain this returned value as long as you want
     *         the time observer to be invoked by the synchronizer. Pass this object to -removeTimeObserver: to cancel
     *         time observation.
     * @param times The times for which the observer requests notification, supplied as an array of NSValues carrying
     *              CMTimes.
     * @param queue The serial queue onto which block should be enqueued. If you pass NULL, the main queue (obtained
     ...
     * timebase. The block is also invoked whenever time jumps and whenever rendering starts or stops.
     * <p>
     * If the interval corresponds to a very short interval in real time, the synchronizer may invoke the block less
     * frequently than requested. Even so, the synchronizer will invoke the block sufficiently often for the client to
     * update indications of the current time appropriately in its end-user interface.
     * <p>
     * Each call to -addPeriodicTimeObserverForInterval:queue:usingBlock: should be paired with a corresponding call to
     * -removeTimeObserver:. Releasing the observer object without a call to -removeTimeObserver: will result in
     * undefined behavior.
     ...
     *         the time observer to be invoked by the synchronizer. Pass this object to -removeTimeObserver: to cancel
     *         time observation.
     * @param interval The interval of invocation of the block during normal rendering, according to progress of the
     ...
     * This method can be called while rate is non-0.0.
     * <p>
     * time is used to schedule future removals. If the time is in the past, the renderer will be removed immediately.
     * kCMTimeInvalid can also be used to force immediate removal.
     * <p>
     * This method removes the renderer asynchronously. The method can be called more than once, with a subsequent
     * scheduled removal replacing a previously scheduled removal.
     * <p>
     * Clients may provide an optional completionHandler block to be notified when the scheduled removal completes. If
     * provided, completionHandler will always be called with the following values for didRemoveRenderer:
     * <p>
     * - If the renderer has not been added to this synchronizer, completionHandler will be called and didRemoveRenderer
     * will be NO.
     * - If a removal of a particular renderer is scheduled after another removal of that same renderer has already been
     * scheduled but not yet occurred, the previously-scheduled removal's completionHandler will be called and
     * didRemoveRenderer will be NO. The new scheduled removal's completionHandler will not be called until it is
     ...
     *
     * @param renderer          An object conforming to AVQueuedSampleBufferRendering currently synchronized by this
     ...
     * @param completionHandler Optional. A block called when the renderer is removed from the synchronizer. If
     ...
     * calling thread and the queue used to add the time observer, an in-flight block may continue to execute after this
     * method returns. You can guarantee synchronous time observer removal by enqueuing the call to -removeTimeObserver:
     * on that queue. Alternatively, call dispatch_sync(queue, ^{}) after -removeTimeObserver: to wait for any in-flight
     * blocks to finish executing. -removeTimeObserver: should be used to explicitly cancel each time observer added
     * using -addPeriodicTimeObserverForInterval:queue:usingBlock: and
     * -addBoundaryTimeObserverForTimes:queue:usingBlock:.
     *
     * @param observer An object returned by a previous call to -addPeriodicTimeObserverForInterval:queue:usingBlock: or
     *                 -addBoundaryTimeObserverForTimes:queue:usingBlock:.
     */
    @Generated
     ...
     * <p>
     * If no AVSampleBufferAudioRenderer has been added, the source clock will be the host time clock
     * (mach_absolute_time with the appropriate timescale conversion; this is the same as Core Animation's
     * CACurrentMediaTime).
     * <p>
     ...
     * added renderer is NO. If set to NO, playback will attempt to start immediately regardless of the value of
     * hasSufficientMediaDataForReliablePlaybackStart of added renderers. Default is YES.
     */
    @Generated
     ...
     * In other words: if hostTime is in the past, the timebase's time will be interpolated as though the timebase has
     * been running at the requested rate since that time. If hostTime is in the future, the timebase will immediately
     * start running at the requested rate from an earlier time so that it will reach the requested time at the
     * requested hostTime.
     * It is a responsibility of the client to ensure that proper time and hostTime is set. This method will not attempt
     * to validate improper time, hostTime values.
     * In addition, it is also the caller’s responsibility to enqueue samples in the connected renderers that match the
     * timeline defined here.
