...
export BREW_GCC=/usr/local/opt/gcc
check_error [ -d "$BREW_GCC" ]
export BREW_GMP=/usr/local/opt/gmp
check_error [ -d "$BREW_GMP" ]
export BREW_MPFR=/usr/local/opt/mpfr
check_error [ -d "$BREW_MPFR" ]
export BREW_MPC=/usr/local/opt/libmpc
check_error [ -d "$BREW_MPC" ]
export BREW_CLOOG=/usr/local/opt/cloog
check_error [ -d "$BREW_CLOOG" ]
export BREW_ISL=/usr/local/opt/isl
check_error [ -d "$BREW_ISL" ]
export BREW_WGET=/usr/local/opt/wget
check_error [ -d "$BREW_WGET" ]
export BREW_WINE=/usr/local/opt/wine
check_error [ -d "$BREW_WINE" ]
...
CC=gcc-5 CXX=g++-5 CPP=cpp-5 LD=gcc-5
...
check_error check_download "gcc-5.3.0.tar.bz2" curl -O ftp://gcc.gnu.org/pub/gcc/releases/gcc-5.3.0/gcc-5.3.0.tar.bz2

check_error check_unzip "gcc-5.3.0" tar xjf gcc-5.3.0.tar.bz2
...
check_error cd $DIR/source/gcc-5.3.0
check_error mkdir build32
check_error cd build32

CC=gcc-5 CXX=g++-5 CPP=cpp-5 LD=gcc-5 PATH=/usr/local/mingw/bin/:$PATH

check_error ../configure --target=i686-w64-mingw32 --disable-multilib --enable-languages=c,c++,objc,obj-c++ --with-gmp=$BREW_GMP --with-mpfr=$BREW_MPFR --with-mpc=$BREW_MPC --with-cloog=$BREW_CLOOG --with-isl=$BREW_ISL --with-system-zlib --enable-version-specific-runtime-libs --enable-libstdcxx-time=yes --enable-stage1-checking --enable-checking=release --enable-lto --enable-threads=win32 --prefix=$PREFIX --with-sysroot=$PREFIX
...
check_error ../configure --target=x86_64-w64-mingw32 --disable-multilib --enable-languages=c,c++,objc,obj-c++ --with-gmp=$BREW_GMP --with-mpfr=$BREW_MPFR --with-mpc=$BREW_MPC --with-cloog=$BREW_CLOOG --with-isl=$BREW_ISL --with-system-zlib --enable-version-specific-runtime-libs --enable-libstdcxx-time=yes --enable-stage1-checking --enable-checking=release --enable-lto --enable-threads=win32 --prefix=$PREFIX --with-sysroot=$PREFIX
...
check_error cd $DIR/source/gcc-5.3.0/build32
...
check_error cd $DIR/source/gcc-5.3.0/build64
