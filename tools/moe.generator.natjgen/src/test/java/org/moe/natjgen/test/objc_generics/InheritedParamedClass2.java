...
public class InheritedParamedClass2 extends AbstractObjCClassTest {
    public void test_class_with_templates() {
        TypeDeclaration decl = getClassDecl();
        assertEquals("InheritedParamedClass2", decl.getName().getIdentifier());
        assertEquals(1, decl.typeParameters().size());
        assertEquals("_Y", ((TypeParameter)decl.typeParameters().get(0)).getName().getIdentifier());
    }
    ...
    public void test_superclass_with_templates() {
        ...
        assertEquals("_Y", ((SimpleType)tmp).getName().getFullyQualifiedName());
    }
}
