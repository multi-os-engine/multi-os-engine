 ...
 * 
 * The NEAppProxyProvider class declares the programmatic interface for an object that implements the client side of a
 * custom network proxy solution.
 * 
...
public class NEAppProxyProvider extends NETunnelProvider {
    ...
    public static native NEAppProxyProvider allocWithZone(VoidPtr zone);
    ...
    @Owned
    @Selector("new")
    public static native NEAppProxyProvider new_objc();
     ...
     *         proxy implementation has not retained the flow and will not handle the flow data. In
     *         NETransparentProxyProvider sub-classes returning NO causes the flow to be handled by the networking stack
     *         without any proxy. In all other cases the flow is terminated when NO is returned.
     * 
     *         API-Since: 9.0
     */
    @Generated
