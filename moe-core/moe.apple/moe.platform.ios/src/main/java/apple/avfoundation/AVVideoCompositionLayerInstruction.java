...
/**
 * An AVVideoCompositionLayerInstruction object represents the transform, opacity, and cropping ramps to apply to a
 * given track. Subclasses of this type that are used from Swift must fulfill the requirements of a Sendable type.
 * 
 * API-Since: 4.0
 */
@Generated
...
public class AVVideoCompositionLayerInstruction extends NSObject implements NSSecureCoding, NSCopying,
     ...
     * 
     * - Parameter time: If a ramp with a timeRange that contains the specified time has been set, information about the
     * effective ramp for that time is supplied. Otherwise, information about the first ramp that starts after the
     * specified time is supplied.
     * - Parameter startCropRectangle: A pointer to a CGRect to receive the starting crop rectangle value for the crop
     * rectangle ramp. May be NULL.
     * - Parameter endCropRecrangle: A pointer to a CGRect to receive the ending crop rectangle value for the crop
     * rectangle ramp. May be NULL.
     * - Parameter timeRange: A pointer to a CMTimeRange to receive the timeRange of the crop rectangle ramp. May be
     * NULL.
     * 
     * - Returns: An indication of success. NO will be returned if the specified time is beyond the duration of the last
     ...
     * effective ramp for that time is supplied. Otherwise, information about the first ramp that starts after the
     * specified time is supplied.
     * - Parameter startOpacity: A pointer to a float to receive the starting opacity value for the opacity ramp. May be
     * NULL.
     * - Parameter endOpacity: A pointer to a float to receive the ending opacity value for the opacity ramp. May be
     * NULL.
     ...
     * effective ramp for that time is supplied. Otherwise, information about the first ramp that starts after the
     * specified time is supplied.
     * - Parameter startTransform: A pointer to a float to receive the starting transform value for the transform ramp.
     * May be NULL.
     * - Parameter endTransform: A pointer to a float to receive the ending transform value for the transform ramp. May
     * be NULL.
