...
public class AVCaptureVideoPreviewLayer extends CALayer {
    ...
    public static native AVCaptureVideoPreviewLayer layerWithSession(@NotNull AVCaptureSession session);
    ...
    public static native AVCaptureVideoPreviewLayer layerWithSessionWithNoConnection(@NotNull AVCaptureSession session);
    ...
    public native AVCaptureVideoPreviewLayer initWithCoder(@NotNull NSCoder coder);

    @Generated
    @Selector("initWithLayer:")
    public native AVCaptureVideoPreviewLayer initWithLayer(@NotNull @Mapped(ObjCObjectMapper.class) Object layer);
    ...
    public native AVCaptureVideoPreviewLayer initWithSession(@NotNull AVCaptureSession session);
    ...
    public native AVCaptureVideoPreviewLayer initWithSessionWithNoConnection(@NotNull AVCaptureSession session);
    ...
    @Generated
    @Selector("setSession:")
    public native void setSession(@Nullable AVCaptureSession value);
    ...
    public native void setSessionWithNoConnection(@NotNull AVCaptureSession session);
    ...
    public native AVMetadataObject transformedMetadataObjectForMetadataObject(@NotNull AVMetadataObject metadataObject);
     ...
     * value to `false` if your app needs video preview immediately for startup, and `true` if it does not.
     * 
     * By default, this value is `false` for ``AVCaptureVideoPreviewLayer`` objects, since this object is used to
     * display preview. For best session start performance, set ``deferredStartEnabled`` to `false` for preview layers.
     * If your app contains multiple preview layers, you may want to display the main preview layer as soon as possible
     * and allow the remaining layers to display subsequently. In this case, set ``deferredStartEnabled`` to `true` for
     * the remaining layers.
     * 
     * - Note: Setting this property to the same value for all outputs, including ``AVCaptureVideoPreviewLayer`` and
     * ``AVCaptureOutput``, is equivalent to not using deferred start.
     * 
     * If ``deferredStartSupported`` is `false`, setting this property value to `true` results in the session throwing
