...
```
vendor/svm/labs-openjdk  ──jdk_gensrc──►  build/labsjdk/         ← apply patches, configure, make graal-builder-image
        │                                  │
        │                                  ▼  (Xcode)
        ...
        ▼
vendor/svm/graal  ──build_graal_dist──►  build/graalDist/         ← rsync vendor/svm/graal, mx --java-home <bootJDK> --env ce build, rsync graalvm-home
        │                                  │
        │                                  ▼  (Xcode)
...
vendor/external/javasqlite ──Xcode──►   libjavasqlite.a per arch/sdk
...
- **`svm.graal/`** — runs `mx ... build` against `vendor/svm/graal/` (with `fix_undefined_symbol__clear_cache_apple_clang.patch` applied). Produces a self-contained GraalVM at `build/graalDist/` whose `bin/native-image` is what user-app builds and `svm.jdwp` invoke. `svm.graal.xcodeproj` then builds `libjvm.a`.
...
- **`external.javasqlite/`** — wraps the vendored `vendor/external/javasqlite` C source as an iOS static archive.
...
When a vendor submodule SHA bumps, re-test:

| patch | against |
|---|---|
| `svm.openjdk/add_ios_build_support.patch` | `vendor/svm/labs-openjdk` |
| `svm.openjdk/fix_memset_call.patch` | `vendor/svm/labs-openjdk` |
| `svm.graal/fix_undefined_symbol__clear_cache_apple_clang.patch` | `vendor/svm/graal` |
...
- The runtime `native-image` invoked here (and in user-app builds) is from `vendor/svm/graal` at GraalVM **25**.
