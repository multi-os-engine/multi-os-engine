...
public class ConstNULongPtrTest extends NatJTest {

    private static final long VALUES[] = DataSource.getRawNULongs();
    private static final Long OBJECT_VALUES[] = DataSource.getObjectNULongs();
    ...
    private NULongPtr getTestPtr() {
        return (NULongPtr) NativeDataSource.getNULongTestPtr();
    }

    private NULongPtr getZeroPtr() {
        return (NULongPtr) NativeDataSource.getNULongZeroPtr();
    }

    private void failIfConst(NULongPtr ptr) {
        if (ptr.isConstPtr()) {
    ...
    public void test_ofs_idx() {
        long[] exp = Arrays.copyOfRange(VALUES, SUB_FROM, SUB_TO);
        NULongPtr ofs = getTestPtr().ofs(SUB_FROM);
    ...
    public void test_getGuarded_len() {
        NULongPtr ptr = getTestPtr().getGuarded(NUM_VALUES);
    ...
    public void test_getGuarded_from_to() {
        NULongPtr ptr = getTestPtr().getGuarded(SUB_FROM, SUB_TO);
        Assert.assertNotNull(ptr);
        Assert.assertTrue(ptr.isGuarded());
        Assert.assertTrue(ptr.isConstPtr() == getTestPtr().isConstPtr());
