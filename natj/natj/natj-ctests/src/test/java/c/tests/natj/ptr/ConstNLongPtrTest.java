...
public class ConstNLongPtrTest extends NatJTest {

    private static final long VALUES[] = DataSource.getRawNLongs();
    private static final Long OBJECT_VALUES[] = DataSource.getObjectNLongs();
    ...
    private NLongPtr getTestPtr() {
        return (NLongPtr) NativeDataSource.getNLongTestPtr();
    }

    private NLongPtr getZeroPtr() {
        return (NLongPtr) NativeDataSource.getNLongZeroPtr();
    }

    private void failIfConst(NLongPtr ptr) {
        if (ptr.isConstPtr()) {
    ...
    public void test_ofs_idx() {
        long[] exp = Arrays.copyOfRange(VALUES, SUB_FROM, SUB_TO);
        NLongPtr ofs = getTestPtr().ofs(SUB_FROM);
    ...
    public void test_getGuarded_len() {
        NLongPtr ptr = getTestPtr().getGuarded(NUM_VALUES);
    ...
    public void test_getGuarded_from_to() {
        NLongPtr ptr = getTestPtr().getGuarded(SUB_FROM, SUB_TO);
        Assert.assertNotNull(ptr);
        Assert.assertTrue(ptr.isGuarded());
        Assert.assertTrue(ptr.isConstPtr() == getTestPtr().isConstPtr());
