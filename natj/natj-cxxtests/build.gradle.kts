...
val processCxxTestsTask = tasks.register<JavaExec>("processCxxTests") {
    dependsOn(sourceSets["test"].output)

    classpath = files(
        "../natj-processor/build/libs/natj-processor-${version}.jar",
        "build/classes/test",
        "../build/classes/main",
        "dyntype/build/classes/main",
    )
    mainClass = "org.moe.natj.processor.Main"
    ...
    val logfile = file("build/processor.log")
    doFirst {
        logfile.parentFile.mkdirs()
        val ostream = FileOutputStream(logfile)
...
tasks.withType<Test>().configureEach {
    ...
    testClassesDirs = processCxxTestsTask.get().outputs.files
