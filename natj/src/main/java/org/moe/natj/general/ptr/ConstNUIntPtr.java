...
/**
 * Constant architecture dependent unsigned Long pointer interface.
 * - On a 32-bit platform the element size is 4 bytes
 * - On a 64-bit platform the element size is 8 bytes
 */
public interface ConstNUIntPtr extends ConstPtr<Long> {
    ...
    @Override
    public ConstNUIntPtr ofs(int elemOffset);

    @Override
    public ConstNUIntPtr getGuarded(int length);

    @Override
    public ConstNUIntPtr getGuarded(int fromIndex, int toIndex);
