...
- **`moeExternal`** (`MOEExternal`) — the typed registry of every cross-build artifact path. Every native task and the SDK assembler obtains paths from here; **don't hard-code paths anywhere else**. Field tree: top-level `{root,prebuilts,external,javasqlite,moe}` (paths to `vendor/`, `prebuilts/`, `vendor/external/`, the javasqlite source, and `<moe>/` respectively); typed children `svm.{root,mx,graal,openjdk}`, `natJ.{root,sources,jnipath,ios,mac,win,linux,ndk}`, `llvm.{source,root,jnipath,macos}` (where `llvm.source` is the vendored llvm-project source and `llvm.{root,jnipath,macos}` are the prebuilt libclang output paths), `moeCore.out.{moeCoreJar, moeIosJar, iphoneosLibmoe, iphoneosSvmjdwp, …}`. Add top-level path fields for new vendored dependencies as they start needing build-script references. Auto-populated relative to the rootProject, so every absolute path resolves the same regardless of which composite is the build root.
